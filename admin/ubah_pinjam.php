                                    ...
                                    <div class="card border-0 shadow-lg rounded">
                                        <div class="card-body p-5">
                                            <div class="card-title mb-4 text-center">
                                                <h4 class="mb-0 text-primary">✏️ Form Ubah Peminjaman</h4>
                                            </div>
                                            <hr />
                                            <?php
                                $id = $_GET['id'];
                                $query = mysqli_query($koneksi, "SELECT peminjaman.*, buku.judul, petugas.nama_petugas, pengguna.nama_pengguna
                                FROM peminjaman 
                                ...
                                ?>
                                            <form action="crud_pinjam.php" method="POST" class="row g-3">
                                                ...
                                                <div class="mb-3 col-12">
                                                    <label for="nama_pengguna" class="form-label">👤 Nama
                                                        Pengguna</label>
                                                ...
                                                while ($datapeng = mysqli_fetch_assoc($querypeng)) :
                                                    $selected = ($datapeng['id'] == $data['id_pengguna']) ? "selected" : "";
                                                ?>
                                                        <option value="<?php echo $datapeng['id']; ?>"
                                                            <?php echo $selected; ?>>
                                                            <?php echo htmlspecialchars($datapeng['nama_pengguna']); ?>
                                                        </option>
                                                        <?php endwhile; ?>
                                                    </select>
                                                </div>
                                                <div class="mb-3 col-12">
                                                    ...
                                                    <select name="judul" class="form-select" required id="judul">
                                                        <option value="">Pilih Judul Buku</option>
                                                        <?php
                                                $querybuku = mysqli_query($koneksi, "SELECT * FROM buku");
                                                while ($databuku = mysqli_fetch_assoc($querybuku)):
                                                    $selected = ($databuku['id'] == $data['id_buku']) ? "selected" : "";
                                                ?>
                                                        <option value="<?php echo $databuku['id']; ?>"
                                                            <?php echo $selected; ?>>
                                                            <?php echo htmlspecialchars($databuku['judul']); ?>
                                                        </option>
                                                        <?php endwhile; ?>
                                                    </select>
                                                </div>
                                                <div class="d-flex justify-content-end">
                                                    ...
                                                    <button class="btn btn-primary" type="submit" name="btnUbah">✅
                                                        Simpan</button>
                                                </div>
                                            </form>
                                            <?php } ?>
